...
const oauthPlugin = fp(function (fastify, options, next) {
	...
	const cbk = async function (o, code, callback) {
		const result = await o.oauth2.authorizationCode.getToken({
			code: code,
			redirect_uri: callbackUri,
			scope: scope,
		});

		const accessToken = o.oauth2.accessToken.create(result);
	...
	function getNewAccessTokenUsingRefreshTokenCallbacked(
		...
		const accessToken = fastify[name].oauth2.accessToken.create({
